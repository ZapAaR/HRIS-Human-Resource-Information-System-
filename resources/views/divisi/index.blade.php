...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1">Kelola data divisi perusahaan Anda</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
        ...
        <div class="p-4 lg:p-5 border-b border-[#E2E8F0] flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <form action="{{ route('divisi.index') }}" method="GET" class="relative flex-1 max-w-md">
            ...
            <table class="w-full text-sm">
                <thead class="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                    <tr>
                        ...
                        <th class="px-4 lg:px-6 py-3.5 text-left font-semibold text-[#1E293B]">Nama Divisi</th>
                        <th class="px-4 lg:px-6 py-3.5 text-left font-semibold text-[#1E293B]">Deskripsi</th>
                        <th class="px-4 lg:px-6 py-3.5 text-right font-semibold text-[#1E293B] w-40">Aksi</th>
                    </tr>
                </thead>
        ...
        @if($divisis->hasPages())
            <div class="px-4 lg:px-6 py-4 border-t border-[#E2E8F0] flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <nav class="flex items-center gap-1">
                    ...
                    @else
                        <a href="{{ $divisis->previousPageUrl() }}" class="px-3 py-1.5 text-sm text-slate-600 hover:bg-[#F8FAFC] rounded-lg transition">Previous</a>
                    @endif
...
(function() {
    ...
    // ============================================================
    // OPEN DELETE MODAL - Set action URL
    // ============================================================
    ...
    @if($errors->has('nama_divisi') || $errors->has('deskripsi'))
        @if(old('_method') === 'PUT')
            // Edit validation error - reopen edit modal
            openModal(modalEdit);
        @else
            // Create validation error - reopen create modal
            openModal(modalCreate);
        @endif
    @endif
