    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-[#1E293B]">Tambah User</h1>
                    <p class="text-slate-500 mt-1">Buat akun user baru dengan role yang sesuai</p>
                </div>
        ...
        <form action="{{ route('user.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" novalidate>
            @csrf
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}"
                            ...
                            class="w-full px-4 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-sm text-[#1E293B] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition @error('name') border-red-400 @enderror">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email') }}"
                            ...
                            class="w-full px-4 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-sm text-[#1E293B] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition @error('email') border-red-400 @enderror">
                        @error('email')
                            <p class="mt-1.5 text-xs text-red-500 flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor">
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-medium text-[#1E293B] mb-1.5">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            @foreach ($roles as $role)
                                ...
                                <label class="flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer">
                                    <input type="radio" name="role" value="{{ $role->name }}"
                                        {{ $checked ? 'checked' : '' }} class="w-4 h-4">
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit"
                    class="px-6 py-2.5 bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-sm font-medium rounded-xl shadow-sm shadow-blue-600/20 transition">Simpan</button>
            </div>
        </form>
    </div>
