...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <nav class="flex items-center gap-2 text-sm text-slate-500 mb-2 flex-wrap">
                    ...
                    <span class="text-[#1E293B] font-medium">Manajemen User</span>
                </nav>
                <h1 class="text-2xl lg:text-3xl font-bold text-[#1E293B]">Manajemen User</h1>
                <p class="text-slate-500 mt-1 text-sm lg:text-base">Kelola user dan hak akses sistem</p>
            </div>
        </div>

        {{-- Success Alert --}}
        @if (session('success'))
            <div class="flex items-start gap-3 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl"
                role="alert">
                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor">
        ...
        <form method="GET" class="flex flex-col sm:flex-row gap-3 w-full">
            ...
            <select name="role" onchange="this.form.submit()"
                class="w-full sm:w-56 lg:w-64 rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
            ...
            <button type="submit"
                class="w-full sm:w-36 lg:w-40 px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition">
                Filter
            </button>

            @if (request()->filled('search') || request()->filled('role'))
                <a href="{{ route('user.index') }}"
                    class="w-full sm:w-36 lg:w-40 px-5 py-2.5 rounded-xl border border-slate-300 hover:bg-slate-100 text-center text-sm font-medium text-slate-700 transition">
                    Reset
                </a>
            @endif
        ...
        <div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
            ...
            <div class="p-4 lg:p-5 border-b border-[#E2E8F0] flex flex-col gap-3">
                <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between w-full">
                    <form action="{{ route('user.index') }}" method="GET" class="relative w-full sm:max-w-md">
            ...
            <div class="lg:hidden divide-y divide-[#E2E8F0]">
                    ...
                    <div class="p-4 space-y-3">
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="font-semibold text-[#1E293B] truncate">{{ $user->name }}</p>
                                    <p class="text-xs text-slate-500 truncate">{{ $user->email }}</p>
                                </div>
                            </div>
                            <span
                                class="text-xs text-slate-400 whitespace-nowrap">{{ $user->created_at->format('d M Y') }}</span>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            @forelse($user->roles as $role)
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-md border {{ $color }}">
                        ...
                        <div class="grid grid-cols-3 gap-2 pt-2">
                            <a href="{{ route('user.show', $user) }}"
                                class="flex flex-col items-center justify-center gap-1 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-xs font-medium transition">
                            ...
                            <a href="{{ route('user.edit', $user) }}"
                                class="flex flex-col items-center justify-center gap-1 py-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg text-xs font-medium transition">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <button type="button"
                                class="btn-delete flex flex-col items-center justify-center gap-1 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-xs font-medium transition"
                                data-action="{{ route('user.destroy', $user) }}" data-nama="{{ $user->name }}">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        <div
                            class="w-16 h-16 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center mx-auto mb-4">
                        ...
                        <h3 class="text-base font-semibold text-[#1E293B] mb-1">Belum ada user</h3>
                        <p class="text-sm text-slate-500 mb-4">Mulai tambahkan user pertama Anda</p>
                    </div>
                @endforelse
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">User</th>
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Role</th>
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Bergabung</th>
                            ...
                            <tr class="hover:bg-[#F8FAFC] transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <p class="font-medium text-[#1E293B] truncate">{{ $user->name }}</p>
                                            <p class="text-xs text-slate-500 truncate">{{ $user->email }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="button"
                                            ...
                                            data-action="{{ route('user.destroy', $user) }}"
                                            data-nama="{{ $user->name }}">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @if ($users->hasPages())
                <div
                    class="px-4 lg:px-6 py-4 border-t border-[#E2E8F0] flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
                    <p class="text-xs sm:text-sm text-slate-500">
                        Menampilkan <span class="font-medium text-[#1E293B]">{{ $users->firstItem() }}</span>
                        sampai <span class="font-medium text-[#1E293B]">{{ $users->lastItem() }}</span>
                        dari <span class="font-medium text-[#1E293B]">{{ $users->total() }}</span> data
                    </p>
                    <nav class="flex items-center gap-1 flex-wrap justify-center">
                        ...
                        @else
                            <a href="{{ $users->previousPageUrl() }}"
                                class="px-3 py-1.5 text-sm text-slate-600 hover:bg-[#F8FAFC] rounded-lg transition">Prev</a>
                        @endif

                        @foreach ($users->getUrlRange(1, $users->lastPage()) as $page => $url)
                            @if ($page == $users->currentPage())
                                <span
                                    class="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-sm font-medium bg-[#2563EB] text-white rounded-lg">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}"
                                    class="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-sm text-slate-600 hover:bg-[#F8FAFC] rounded-lg transition">{{ $page }}</a>
                            @endif
                        @endforeach
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div
                ...
                <div class="flex items-center justify-between border-b border-[#E2E8F0] px-6 py-4">
                    <h3 class="text-lg font-semibold text-[#1E293B]">Hapus User</h3>
                    <button type="button"
                ...
                <form id="form-delete" action="" method="POST" class="px-6 py-5">
                    ...
                    <div class="flex flex-col items-center text-center py-2">
                        ...
                        <p class="text-sm text-slate-600 leading-relaxed">
                            Apakah Anda yakin ingin menghapus user <strong id="delete-nama"
                                class="text-[#1E293B]"></strong>? Semua data dan akses user ini akan dihapus permanen.
                        </p>
                    </div>
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-2 pt-4 border-t border-[#E2E8F0]">
                        <button type="button"
                            class="btn-close-modal w-full sm:w-auto px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition mb-2 sm:mb-0">Batal</button>
                        <button type="submit"
                            class="w-full sm:w-auto px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded-xl shadow-sm shadow-red-600/20 transition">Hapus</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        (function() {
            ...
            // Event listener untuk semua tombol delete (baik di mobile card maupun desktop table)
