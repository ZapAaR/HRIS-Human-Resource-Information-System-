    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 mt-1 text-sm lg:text-base">Kelola data karyawan perusahaan</p>
            </div>
        </div>
        ...
        {{-- Filter Form --}}
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3 w-full">
            <select name="status" class="w-full rounded-lg border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
                <option value="">Semua Status</option>
                <option value="Aktif" @selected(request('status') == 'Aktif')>Aktif</option>
                <option value="Tidak Aktif" @selected(request('status') == 'Tidak Aktif')>Tidak Aktif</option>
            </select>

            <select name="divisi_id" class="w-full rounded-lg border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
            ...
            <select name="posisi_id" class="w-full rounded-lg border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
            ...
            <select name="jenis_kelamin" class="w-full rounded-lg border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white">
            ...
            <div class="md:col-span-4 flex flex-col sm:flex-row gap-2">
                <button class="w-full sm:w-auto px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition">
                    Filter
                </button>
                <a href="{{ route('karyawan.index') }}" class="w-full sm:w-auto px-5 py-2.5 bg-slate-200 text-sm font-medium text-slate-700 rounded-lg hover:bg-slate-300 transition text-center">
                    Reset
                </a>
            </div>
        </form>
        ...
        <div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
            ...
            <div class="p-4 lg:p-5 border-b border-[#E2E8F0] flex flex-col gap-3">
                <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between w-full">
                    <form action="{{ route('karyawan.index') }}" method="GET" class="relative w-full sm:max-w-md">
            ...
            <div class="lg:hidden divide-y divide-[#E2E8F0]">
                    ...
                    <div class="p-4 space-y-3">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-[#1E293B] truncate">{{ $karyawan->nama_karyawan }}</p>
                            ...
                            @php
                                ...
                                $statusColor = $statusColors[$karyawan->status] ?? 'bg-slate-50 text-slate-700 border-slate-200';
                            @endphp
                            <span class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-md border {{ $statusColor }} whitespace-nowrap">
                                {{ $karyawan->status }}
                            </span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 text-sm">
                            <div class="bg-slate-50 rounded-lg px-3 py-2">
                                ...
                                <p class="font-medium text-[#1E293B] text-xs truncate">{{ $karyawan->divisi->nama_divisi ?? '-' }}</p>
                            </div>
                            <div class="bg-slate-50 rounded-lg px-3 py-2">
                                <p class="text-xs text-slate-500">Posisi</p>
                                <p class="font-medium text-[#1E293B] text-xs truncate">{{ $karyawan->posisi->nama_posisi ?? '-' }}</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-3 gap-2 pt-1">
                            <a href="{{ route('karyawan.show', $karyawan) }}" class="flex flex-col items-center justify-center gap-1 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-xs font-medium transition">
                            ...
                            <a href="{{ route('karyawan.edit', $karyawan) }}" class="flex flex-col items-center justify-center gap-1 py-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg text-xs font-medium transition">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                                Edit
                            </a>
                            <button type="button" class="btn-delete flex flex-col items-center justify-center gap-1 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-xs font-medium transition" data-action="{{ route('karyawan.destroy', $karyawan) }}" data-nama="{{ $karyawan->nama_karyawan }}">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        ...
                        <h3 class="text-base font-semibold text-[#1E293B] mb-1">Belum ada data karyawan</h3>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Karyawan</th>
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Divisi</th>
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Posisi</th>
                            <th class="px-6 py-3.5 text-left font-semibold text-[#1E293B]">Status</th>
                            ...
                            <tr class="hover:bg-[#F8FAFC] transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <p class="font-medium text-[#1E293B] truncate">{{ $karyawan->nama_karyawan }}</p>
                                            <p class="text-xs text-slate-500 truncate">{{ $karyawan->email }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-slate-600">{{ $karyawan->divisi->nama_divisi ?? '-' }}</td>
                                <td class="px-6 py-4 text-slate-600">{{ $karyawan->posisi->nama_posisi ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusColors = [
                                            'Aktif' => 'bg-green-50 text-green-700 border-green-200',
                                        ...
                                        $statusColor = $statusColors[$karyawan->status] ?? 'bg-slate-50 text-slate-700 border-slate-200';
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-lg border {{ $statusColor }}">
                                        {{ $karyawan->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('karyawan.show', $karyawan) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 hover:text-blue-700 text-xs font-medium rounded-lg border border-blue-200 transition-all duration-200">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            </svg>
                                        ...
                                        <a href="{{ route('karyawan.edit', $karyawan) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-600 hover:text-amber-700 text-xs font-medium rounded-lg border border-amber-200 transition-all duration-200">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                            <span>Edit</span>
                                        </a>
                                        <button type="button" class="btn-delete inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 text-xs font-medium rounded-lg border border-red-200 transition-all duration-200" data-action="{{ route('karyawan.destroy', $karyawan) }}" data-nama="{{ $karyawan->nama_karyawan }}">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                            </svg>
                                            <span>Hapus</span>
            ...
            @if ($karyawans->hasPages())
                <div class="px-4 lg:px-6 py-4 border-t border-[#E2E8F0] flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
                    ...
                    <nav class="flex items-center gap-1 flex-wrap justify-center">
                        ...
                        @else
                            <a href="{{ $karyawans->previousPageUrl() }}" class="px-3 py-1.5 text-sm text-slate-600 hover:bg-[#F8FAFC] rounded-lg transition">Prev</a>
                        @endif
                        ...
                        @if ($karyawans->hasMorePages())
                            <a href="{{ $karyawans->nextPageUrl() }}" class="px-3 py-1.5 text-sm text-slate-600 hover:bg-[#F8FAFC] rounded-lg transition">Next</a>
                        @else
                ...
                <form id="form-delete" action="" method="POST" class="px-6 py-5">
                    ...
                    <div class="flex flex-col items-center text-center py-2">
                        ...
                        <p class="text-sm text-slate-600 leading-relaxed">
                            Apakah Anda yakin ingin menghapus karyawan <strong id="delete-nama" class="text-[#1E293B]"></strong>? Data yang dihapus tidak dapat dikembalikan.
                        </p>
                    </div>
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-2 pt-4 border-t border-[#E2E8F0]">
                        <button type="button" class="btn-close-modal w-full sm:w-auto px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition mb-2 sm:mb-0">Batal</button>
                        <button type="submit" class="w-full sm:w-auto px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded-xl shadow-sm shadow-red-600/20 transition">Hapus</button>
                    </div>
                </form>
            </div>
        </div>
